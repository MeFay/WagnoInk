  ...
  >
    {[
      ...
      <MotionBox
        ...
        sx={{ width: line.w, height: "1.5px", borderRadius: 999, bgcolor: line.amber ? alpha(theme.palette.accent.main, 0.33) : "rgba(255,255,255,0.1)", transformOrigin: "right" }}
      />
    ))}
  </MotionBox>
  );
    ...
    <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap" }}>
      {categories.map((cat) => (
        <Box key={cat} component="button" onClick={() => onChange(cat)}
          sx={{
            background: "none",
            border: active === cat ? `1px solid ${alpha(theme.palette.primary.main, 0.6)}` : "1px solid rgba(255,255,255,0.15)",
            cursor: "pointer", px: 2, py: 0.75, borderRadius: 999,
            fontSize: typeScale.caption, fontWeight: 600, letterSpacing: 0.5,
            color: active === cat ? "white" : "text.secondary",
            bgcolor: active === cat ? alpha(theme.palette.primary.main, 0.15) : "transparent",
            fontFamily: "inherit",
...
const FeaturedCard = ({ post }) => {
  ...
  <MotionBox
    ...
    sx={{
      ...
      "&:hover": { borderColor: alpha(theme.palette.primary.main, 0.3), boxShadow: "0 32px 60px rgba(0,0,0,0.5)" },
...
const PostCard = ({ post, index }) => {
  ...
  <MotionBox
    ...
    sx={{
      ...
      "&:hover": { transform: "translateY(-6px)", borderColor: alpha(theme.palette.primary.main, 0.3), boxShadow: "0 24px 48px rgba(0,0,0,0.5)" },
...
const Blog = () => {
  ...
  return (
    <Box id="page-blog" sx={{ minHeight: "100vh", pt: { xs: "88px", md: "96px" }, pb: { xs: 6, md: 8 }, px: { xs: 3, sm: 5, md: 8, lg: 10 } }}>
      <Box sx={{ maxWidth: 1400, mx: "auto", display: "flex", flexDirection: "column", gap: { xs: 6, md: 8 } }}>

        {/* ── Header ── */}
        <Box id="blog-header" sx={{ display: "flex", alignItems: "center", gap: { md: 6 }, pt: { xs: 6, md: 8 } }}>
          <MotionBox
